...
export const removeCredits = TryCatch(async (req: Request, res: Response) => {
  try {
    const { user: requestBodyUser, amount } = req.body;
    ...
    const creditsToDeduct = amount && amount > 0 ? amount : 5; // Default to 5 if amount is not specified
    ...
    res.status(200).json({ 
      success: true, 
      message: `${creditsToDeduct} credits removed`, 
